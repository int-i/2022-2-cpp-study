...
// 상위 클래스의 포인터 객체나 레퍼런스(참조 변수) 객체 등이 하위 클래스의 객체를 참조하게 하는 것이다.
// 
...
// 탈것 포인터 변수는 자료형이 탈것 인 객체의 주소값만을 담을 수 있다.
탈것* 탈것포인터{ new 탈것{ } }; // 탈것 포인터 객체

탈것* 탈것포인터2{ new 배{ } };
탈것* 탈것포인터3{ new 자동차{ } };
탈것* 탈것포인터4{ new 자전거{ } };

// 이런식으로 is-a 관계의 경우 업스케일링이 가능하기 때문에 위와 같은 코드가 성립한다.

// 익명 int형 변수를 동적할당을 하고, 그 변수의 주소값을 p가 받는다.
// 동적 할당: new라는 키워드를 통해서 heap 메모리에 변수를 선언하는 것
// 동적 할당을 하는 이유: 기본적으로 main함수 안에서 선언하는 변수는
// 런타임에 크기 변경이나, 할당 해제 등을 할 수가 없다.
// 하지만 동적할당 된 변수는 런타임에 변수가 할당된 공간을 늘리거나 줄이는게 가능하고,
// 사용하지 않는 공간은 프로그래머의 판단 하에 해제도 가능하다.

// new: heap 영역에 변수를 선언하고 그 변수의 주소값을 반환함

// 컴파일 타임에만 크기 수정이 가능함
//int arr[12] = { };

// 런타임에도 크기 수정이 가능함
// 런타임 중에 메모리 공간을 반환하고, 내가 원하는 크기 만큼 메모리 공간 재할당이 가능하기 때문
//int* arr = new int[32] {};
//delete[] arr;
//arr = new int[10] {};
...
// 
// 바인딩: 프로그래머가 ​코딩을 해서 컴파일을 하게 되면 각각의 코드가 실행시간에 메모리 어딘가에 저장되고,
